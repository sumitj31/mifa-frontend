...
const form = reactive<any>({
  poDate: today, yourReference: '', ourReference: '', transactionType: 'Domestic', natureOfSupply: 'Goods', vendorId: '', vendorContactId: '', deliveryLocationId: '', sprListId: '', paymentTerms: 'AGAINST PROFORMA INVOICE', dispatchThrough: 'COURIER', destination: 'AHMEDABAD', comments: '', deliveryInstructions: '', freight: 0, amountPaid: 0, items: [],
})
...
function selectProduct(item: any) {
  ...
  Object.assign(item, {
    ...
    abpPrice: Number(product.abpPrice ?? 0),
    unitPrice: Number(product.unitPrice ?? 0),
    tradeDiscountPct: Number(product.tradeDiscountPct ?? 0),
  })
}
...
function onProductSelected(item: any, product: any) {
  ...
  item.abpPrice = Number(product.abpPrice ?? 0)
  item.unitPrice = Number(product.unitPrice ?? 0)
  item.tradeDiscountPct = Number(product.tradeDiscountPct ?? 0)
...
function onProductChange(item: any) {
  ...
  item.tradeDiscountPct = product.tradeDiscountPct
...
async function submit() {
  ...
  try {
    const order = await api<any>('/orders', { method: 'POST', body: {
      poDate: form.poDate, yourReference: form.yourReference || undefined, ourReference: form.ourReference || undefined,
...
<template>
  <form class="order-builder" @submit.prevent="submit">
    <PageHeader eyebrow="Order builder" title="Create Purchase Order" description="Select an SPR revision or enter a quoted non-SPR line. Calculations are snapshotted when the PO is saved.">
      <NuxtLink to="/orders" class="btn btn-secondary">Cancel</NuxtLink><button class="btn btn-primary" :disabled="saving">{{ saving ? 'Creating…' : 'Create Purchase Order' }}</button>
    </PageHeader>
    ...
    <div style="display:flex;justify-content:flex-end;gap:10px"><NuxtLink to="/orders" class="btn btn-secondary">Cancel</NuxtLink><button class="btn btn-primary" :disabled="saving">{{ saving ? 'Creating order…' : 'Create Purchase Order' }}</button></div>
  </form>
</template>
